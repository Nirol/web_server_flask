...
from logic.kalfi_display import _query_db_kalfi_data_all, \
    _sort_lists_by_kalfi_num
...
class TestKalfiMeta(unittest.TestCase):
    ...
    def test_kalfi_metadata_query_working(self):
        ...
        for yeshuv in yeshuv_sn_list:
            [kalfi_data_list, kalfi_meta_list] = _query_db_kalfi_data_all(yeshuv.yeshuv_sn)
            _sort_lists_by_kalfi_num(kalfi_data_list, kalfi_meta_list)
            result = validate_list_match_test_db(kalfi_data_list, kalfi_meta_list)
            self.assertEqual(result, [])
            # if result:
            #     print("result for sn={}".format(yeshuv.yeshuv_sn))
            #     print(result)
...
def validate_list_match_test_db(kalfi_data_list: List[Knesset_22],
    ...
    if len_data_list != len_meta_list:
        ...
        for idx in range(len_meta_list):
            ...
            kalfi_num_meta_double = kalfi_meta_list[idx].kalfi_num
